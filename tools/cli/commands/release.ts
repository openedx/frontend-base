...
export default function release() {
  ...
  if (!existsSync(tsconfigPath)) {
    console.error(chalk.red('openedx release: the library must include a tsconfig.json. Aborting.'));
    process.exit(1);
  }
  ...
  execSync(`tsc --project ${path.resolve(process.cwd(), './tsconfig.json')}`, { stdio: 'inherit' });
