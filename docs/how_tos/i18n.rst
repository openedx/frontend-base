...
#. Add ``@openedx/frontend-base`` as a dependency to your ``package.json`` . (If you are actually writing a consumable component, add ``@openedx/frontend-base`` as both a dev dependency and peer dependency instead.)  ``@openedx/frontend-base`` re-exports everything from ``react-intl`` plus additional helpers. You should only access the ``react-intl`` functions and elements exposed by ``@openedx/frontend-base``.
