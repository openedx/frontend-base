...
The menu is one component, ``shell/menus/LanguageMenu``, registered as a widget in three slots:

- ``org.openedx.frontend.slot.header.desktopRight.v1`` (widget ``...header.desktopLanguageMenu.v1``)
- ``org.openedx.frontend.slot.header.mobileRight.v1`` (widget ``...header.mobileLanguageMenu.v1``)
- ``org.openedx.frontend.slot.footer.desktopRightLinks.v1`` (widget ``...footer.desktopRightLinksLanguageMenu.v1``)

The two header registrations put it within reach at every viewport, including on course units and Studio editing views where the footer is off screen. Because each is a widget rather than part of the header layout, operators can move or remove any of them with the usual slot operations.

The toggle is marked with a globe icon and, on viewports at ``md`` and above, the current language's name beside it. Narrower viewports show the globe alone, since the rest of the top bar matters more there; the toggle's ``aria-label`` names the current language at every width.
